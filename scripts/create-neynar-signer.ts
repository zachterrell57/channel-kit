...
if (!NEYNAR_API_KEY || !FARCASTER_MNEMONIC) {
  throw new Error("Please set NEYNAR_API_KEY and FARCASTER_MNEMONIC in your .env file");
}
...
async function getFid() {
  ...
  const { user: farcasterDeveloper } = await neynarClient.lookupUserByCustodyAddress(account.address);
    ...
    try {
      ...
      if (signer.status === "approved") {
        ...
        if (signerUuidRegex.test(envContent)) {
          envContent = envContent.replace(signerUuidRegex, `SIGNER_UUID=${signerUuid}`);
        } else {
...
async function createSigner() {
  try {
    ...
    const signedKey = await neynarClient.registerSignedKey(createSigner.signer_uuid, fid, deadline, signature);
